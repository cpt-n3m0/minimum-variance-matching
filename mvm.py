...
ffi.cdef("""
    ...
    match_t; match_t mvm(float* query, int lenq, float* target, int lent, int elasticity); 
""")
mvmlib = ffi.dlopen("./libmvm.so")

def mvm(query, target, elasticity=50):
    lq = len(query)
    lt = len(target)
    query = ffi.new(f"float[{lq}]", query)
    target = ffi.new(f"float[{lt}]", target)
    res = ffi.new("match_t *")
    res = mvmlib.mvm(query, lq, target, lt, elasticity)
